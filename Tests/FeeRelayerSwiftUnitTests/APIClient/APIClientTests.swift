...
class APIClientTests: XCTestCase {
    ...
    func testGetFreeTransactionFeeLimit() async throws {
        let feeRelayer = APIClientMock()
        let result = try await feeRelayer.getFreeFeeLimits(for: "GZpacnxxvtFDMg16KWSH8q2g8tM7fwJvNMkb2Df34h9N")
...
private class APIClientMock: FeeRelayerSwift.APIClient {

    // MARK: - Initializers

    public init(httpClient: HTTPClient = FeeRelayerHTTPClient(networkManager: MockNetworkManager())) {
        super.init(httpClient: httpClient, baseUrlString: "", version: 1)
    }
}
...
private class SolanaAPIClientMock: SolanaAPIClient {
